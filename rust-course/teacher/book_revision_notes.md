...
- 章节：`rust-course/src/basic/flow-control.md`
  - 问题：新手容易把 `if` 仍然按其它语言的习惯只当成“语句”，从而不理解为什么它可以直接出现在 `let x = if ... { ... } else { ... };` 的右边。
  - 建议：在 `if` 章节最前面加一句高亮总结：Rust 中 `if` 可以是表达式，因此能够返回值参与赋值；再配一个“赋值成功 / 因分支类型不一致而失败”的并排对照例子。

- 章节：`rust-course/src/basic/flow-control.md`
  - 问题：新手容易知道 `for item in collection`、`for item in &collection`、`for item in &mut collection` 写法不同，但说不清三者到底在所有权上差在哪里，尤其容易忽略数组元素是否 `Copy` 会影响循环后原值能否继续使用。
  - 建议：在 `for` 循环部分补一个更贴近初学者的对照：`[i32; 3]` 与 `[String; 2]` 分别用 `for item in collection` 遍历，展示前者循环后仍可用、后者会 move 掉；帮助把“语法差异”连到“所有权差异”。

- 章节：`rust-course/src/basic/compound-type/array.md`
  - 问题：新手容易把 `[i32; 5]` 和 `[3; 5]` 混在一起，前者是类型写法，后者是数组值的重复简写；尤其容易误以为方括号里的第一个位置总是在写类型。
  - 建议：在数组章节里增加一个并排对照：`let a: [u8; 4] = [1; 4];`，明确标注“左边是类型、右边是值”，并把 `[3; 5]` 直接展开成 `[3, 3, 3, 3, 3]` 帮助建立直觉。

- 章节：`rust-course/src/basic/compound-type/array.md`
  - 问题：新手容易知道“数组越界不行”，但说不清到底是编译时报错还是运行时 panic，也容易把“明显越界”误解成“总会编译不过”。
  - 建议：在索引部分补一句更精确的总结：Rust 数组索引会做边界检查，越界时会 panic；再配一个 `let i = 3; a[i]` 的最短例子，让读者把“边界检查发生在访问时”这件事想清楚。
